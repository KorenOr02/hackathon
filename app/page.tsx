...
export default function Home() {
  ...
  useEffect(() => {
    if (!started || activeStep < 4 || complete || !callIds.length) return;
  ...
  }, [started, activeStep, complete, callIds]);
  ...
  async function startNegotiation() {
    ...
    try {
      ...
      setStarted(true);
      setCallIds(result.calls.map((call: { callId: string }) => call.callId));
      setCallMessage(`${result.message} כל השיחות נשלחות כרגע ליעד בדיקות משותף.`);
    ...
    <BankShell title="חיסכון חכם" subtitle="Swaper מזהה חריגות, משווה הצעות ומכינה פעולות לאישור" workspace="negotiator">
      <div className="negotiator-workspace">
        <section className="negotiator-intro">
          <div><span className="bank-kicker">הזדמנות חיסכון חדשה</span><h2>חשבון הסלולר עלה.<br /><em>Swaper כבר מטפלת בזה.</em></h2><p>זוהתה עלייה חריגה בחיוב הקבוע של סלקום. ניתן להפעיל סוכן משא ומתן ולהשוות הצעות, ללא ביצוע פעולה אמיתית.</p></div>
        ...
        {started && <div id="negotiation" className="negotiation-grid"><NegotiationTimeline activeStep={activeStep} complete={complete} summarySent={summarySent} />{complete && <BestOfferCard summarySent={summarySent} />}</div>}
      </div>
    </BankShell>
  );
}
